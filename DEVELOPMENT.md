...
**A page turn can slide the next page on** like a card off a deck, which is the `slide` setting and also off by default - it is a fifth of a second before what the reader pressed for can be read. A window of the screen is an image with its own origin, so a page drawn into `screen.window(rect(left, 0, W - left, H))` lands shifted by `left` and clipped to the window: that is the card, and the page underneath is left standing wherever it has not reached, so a frame draws the incoming page and nothing else. 11 frames over 250ms at 13.0ms each, including a clock face as the incoming card. Two things it depends on. `screen` is a *builtin*, so it is rebound for the render rather than passed down - an extension's page renderer resolves the same name, and would otherwise draw its face on the screen while the app drew the card. And a fresh window carries no font, so `label()` restoring `screen.font` raises unless the card is given one first. The card always arrives from the right whichever way the reader is paging: a window clamps to the screen, so there is no drawing at a negative origin to bring one in from the left.
