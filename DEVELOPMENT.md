...
A press answers immediately and the movement follows: `draw.furniture` puts the new title and pip row up on the press, and the slide is scheduled for `SLIDE_WAIT_MS` of quiet afterwards, each further press pushing it out again. So a burst of five is one slide onto the page the pip landed on, where before it was five slides drawing over each other - and the page being slid to is only known once they stop. The body is left standing during the wait, because that is what the movement has to travel away from.
