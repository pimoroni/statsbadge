...
**A plot walks left between readings** under the same `animate` setting, rather than jumping a sample when one lands. `pages.PHASE` is how far through the poll interval the frame is - the app sets it, being what knows when the last reading arrived and how often they come - and `draw.area` lays the series out that far to the left, one sample wider than its box, with `screen.clip` holding it inside. The walk itself is free (thermal 25.39ms walking against 25.41 still); what it costs is that the page is redrawn every frame instead of once a second, the way the waterfall already is. So the graph page runs at 30fps continuously and the trend page at 45.
