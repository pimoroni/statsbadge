...
- **Labels** become sprites. Values churn, so the cache is dropped wholesale past 220 entries or 768KB rather than aged. It earns its keep only where a string is drawn again: a hit saves 0.075ms at 11pt, 0.32ms at 17pt, and a page like the CPU dial takes 8 hits a frame - but a *miss* costs 2.73ms at 44pt against 0.74ms to draw the same text live, and above about 40pt a hit loses too, the sprite being bigger to blit than the glyphs are to rasterise (104pt: 3.01ms to blit, 1.27ms live). A small sprite is 5.6KB of heap.
