...
**`custom` is derived, not written down.** One accent, a light or dark page and a ramp style, in [`derive.py`](src/statsbadge/derive.py), which works in OKLCH so "ink has to be readable on the page" is arithmetic rather than judgement: `ink` and `dim` are placed at the lightnesses the hand-tuned dark and light palettes use and then *checked* against WCAG 7 and 4.5, moving outwards only if a hue misses. Walking outwards from the page until the contrast passes was the first attempt and it lands on the dimmest ink that clears the bar, which looks exactly like what it is. The accent's lightness comes from the mode, so the same hue is darker on a pale page and a swatch shows the colour that will be used. `signal` ramps travel to red the way a warning light does and are withheld from accents already there; `mono` ramps stay in the accent's hue and move *away* from the page, because a ramp that darkens on a dark page has its hot end receding into the background exactly when the reading gets interesting - which is what the shipped `cyan` does, and why its second graph series falls back to grey.

The choices are stored and the palette is derived from them, so a change here reaches a badge that already has one; what travels is a palette like any other, and nothing on the badge knows the difference. Restricted on purpose: twelve accents times two modes times the ramps each allows is 42 combinations, and the test checks every one of them for ink and dim contrast, a hot end visible against the page, ramp ends that differ, and that the badge's own `from_palette` builds it. The maths is duplicated - the firmware has its own - which is the price of the config UI being able to show you the theme before you commit to it; it is checked against the firmware's conversion colour by colour and agrees to within a count.
