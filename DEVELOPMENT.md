...
**A plot walks left between readings** under the same `animate` setting, rather than jumping when an update lands. Three periods are in play and only one of them is the plot's: the host samples every `serve --interval`, the badge polls every `interval_ms`, and it refetches the series on its own timer. `pages.note_sample` takes the pace from `seq` and `t`, which the host moves once per sample, divided by how many went by - so a missed poll does not read as the host slowing down. `SAMPLE_STEPS` is how many samples an update brings, which is not one when the badge polls slower than the host samples: the walk covers all of them and `draw.WALK_LEAD` lays that many past the right edge for them to slide in from, `screen.clip` holding them inside. A plot laid across its width alone would shift left and leave a gap that grows and snaps back, which reads as shrinking. Residual pauses where the refresh is not a multiple of the sample period, up to one sample, and past 12 samples an update, where headroom is capped. The walk itself is free (thermal 25.39ms walking against 25.41 still); what it costs is a frame every frame, so the graph page runs at 30fps continuously and the trend page at 45.
