...
Measured: `over` 12.0ms a frame, `deck` 14.6, ten frames over 250ms, with a clock face as the arriving card. The setup is the expensive part - 45ms to draw a page into an image against 15 on the screen, an image being on the heap in PSRAM where the framebuffer is SRAM, plus 23ms to keep the outgoing page for a deck - and it is paid once on the press.
