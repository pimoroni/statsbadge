...
# What the badge draws between polls, at the rate it draws them: the app polls once a
# second and this page is animated, so a poll is about 28 frames.
FRAMES_PER_POLL = 28
CORES = 12
POLLS = 18
# Which threads stay pegged, and which one waits the whole time. Without them a busy
# machine draws as a solid block.
PEGGED = (3, 9)
WAITING = 5
...
def jitter(seed):
    """Return a repeatable wobble in -3..3, so the shot is the same shot every time."""
    seed = (seed * 1103515245 + 12345) & 0x7FFFFFFF
    return (seed % 600) / 100.0 - 3.0


def load_at(poll, core):
    """Return one thread's load, as a percentage."""
    if core == WAITING:
